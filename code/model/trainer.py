...
class Trainer(object):
    ...
    def entropy_reg_loss(self, all_logits):
        ...
        logits_exp = tf.exp(all_logits)
        entropy_policy = - tf.reduce_mean(
            input_tensor=tf.reduce_sum(input_tensor=tf.multiply(logits_exp, all_logits), axis=1)
        )
    ...
    def initialize(self, restore=None, sess=None):
        ...
        self.decaying_beta = tf.compat.v1.train.exponential_decay(
            self.beta, self.global_step, decay_steps=200, decay_rate=0.90, staircase=False
        )
        self.entity_sequence = []

        # Placeholder for discounted reward tensor
        self.cum_discounted_reward = tf.compat.v1.placeholder(
            tf.float32, [None, self.path_length], name="cumulative_discounted_reward"
        )
        for t in range(self.path_length):
            next_possible_relations = tf.compat.v1.placeholder(
                tf.int32, [None, self.max_num_actions], name=f"next_relations_{t}"
            )
            next_possible_entities = tf.compat.v1.placeholder(
                tf.int32, [None, self.max_num_actions], name=f"next_entities_{t}"
            )
            input_label_relation = tf.compat.v1.placeholder(
                tf.int32, [None], name=f"input_label_relation_{t}"
            )
            start_entities = tf.compat.v1.placeholder(tf.int32, [None], name=f"start_entities_{t}")
        ...
        # Call the agent to compute losses
        (
        ...
        self.prev_state = tf.compat.v1.placeholder(
            tf.float32, self.agent.get_mem_shape(), name="memory_of_agent"
        )
        ...
        self.query_embedding = tf.nn.embedding_lookup(
            params=self.agent.relation_lookup_table, ids=self.query_relation
        )  # [B, 2D]
        layer_state = tf.unstack(self.prev_state, self.LSTM_layers)
        formatted_state = [tf.unstack(s, 2) for s in layer_state]

        self.next_relations = tf.compat.v1.placeholder(
            tf.int32, shape=[None, self.max_num_actions], name="next_relations"
        )
        self.next_entities = tf.compat.v1.placeholder(
            tf.int32, shape=[None, self.max_num_actions], name="next_entities"
        )
        self.current_entities = tf.compat.v1.placeholder(
            tf.int32, shape=[None], name="current_entities"
        )

        with tf.compat.v1.variable_scope("policy_steps_unroll", reuse=tf.compat.v1.AUTO_REUSE):
            (
                self.test_loss,
                test_state,
                self.test_logits,
                self.test_action_idx,
                self.chosen_relation,
            ) = self.agent.step(
                self.next_relations,
                self.next_entities,
                formatted_state,
        ...
        if restore:
            ...
            #     logger.info(f"Restoring model from {restore}")
            self.model_saver.restore(sess, restore)
            # else:
            #     raise TypeError(f"The 'restore' argument must be a string path. Got {type(restore)}.")
        else:
            logger.info("Initializing all global variables.")
            sess.run(tf.compat.v1.global_variables_initializer())


    def initialize_pretrained_embeddings(self, sess):
        if self.pretrained_embeddings_action != '':
            embeddings = np.loadtxt(self.pretrained_embeddings_action)
            _ = sess.run(
                self.agent.relation_embedding_init,
                feed_dict={self.agent.action_embedding_placeholder: embeddings}
            )
        if self.pretrained_embeddings_entity != '':
            embeddings = np.loadtxt(self.pretrained_embeddings_entity)
            _ = sess.run(
                self.agent.entity_embedding_init,
    ...
    def bp(self, cost):
        ...
        with tf.control_dependencies([train_op]):  # TensorFlow 2.4: Ensure train_op completes before proceeding
            self.dummy = tf.constant(0)  # TensorFlow 2.4 compatibility: Ensure this is a valid operation
        return train_op

    def calc_cum_discounted_reward(self, rewards):
        running_add = np.zeros([rewards.shape[0]])  # [B]
        cum_disc_reward = np.zeros([rewards.shape[0], self.path_length])  # [B, T]
        cum_disc_reward[:, self.path_length - 1] = rewards  # Set last time step reward
    ...
    def gpu_io_setup(self):
        ...
        for i in range(self.path_length):
            feed_dict[i][self.input_path[i]] = np.zeros(self.batch_size * self.num_rollouts)  # Placeholder
    ...
    def train(self, sess):
        ...
        for episode in self.train_environment.get_episodes():
            ...
            for i in range(self.path_length):
                ...
                per_example_loss, per_example_logits, idx = sess.run([self.per_example_loss[i], self.per_example_logits[i], self.action_idx[i]],
                                                                     feed_dict=feed_dict[i])  # Change for TF 2.x
            ...
            # backprop
            batch_total_loss, _ = sess.run([self.loss_op, self.dummy],
                                           feed_dict={self.cum_discounted_reward: cum_discounted_reward})  # Change for TF 2.x
            # print statistics
    ...
    def test(self, sess, beam=False, print_paths=False, save_model=True, auc=False):
        ...
        for episode in tqdm(self.test_environment.get_episodes()):
            ...
            for i in range(self.path_length):
                ...
                # TensorFlow 2.x: Replacing `sess.run` with eager execution
                loss, agent_mem, test_scores, test_action_idx, chosen_relation = sess.run(
                    ...
                    feed_dict=feed_dict)  # Change for TensorFlow 2.x: no need for `partial_run`
        ...
        if print_paths:
            ...
            for q in paths:
                j = q.replace("/", "")
                f = open(self.output_dir + '/test_beam/' + j + ".txt", "w")
                for p in paths[q]:
                    f.write(str(p))
                f.close()

        logger.info(
            "[ test (mean rewards) {} ]".format(
                [str(self.max_hits_at_1) + " " + str(all_final_reward_1),
                str(self.max_hits_at_3) + " " + str(all_final_reward_3),
                str(self.max_hits_at_5) + " " + str(all_final_reward_5),
                str(self.max_hits_at_10) + " " + str(all_final_reward_10),
                str(self.max_hits_at_20) + " " + str(all_final_reward_20),
                "auc : " + str(auc)]
            )
        )
...
if __name__ == '__main__':
    ...
    if not options['load_model']:
        with tf.compat.v1.Session() as sess:
            ...
            trainer.initialize(restore=None, sess=sess) #line 1086, error starts from here
            trainer.initialize_pretrained_embeddings(sess)
    ...
    # Re-initialize the trainer for testing
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # Perform the test with beam search
        trainer.test(sess, beam=True, print_paths=True, save_model=False)

        # Perform NELL evaluation if specified
